...
import { MoreHorizontal, Eye, Edit, Trash, Copy, CheckCircle, ChevronUp, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
type SortField = 'code' | 'packageName' | 'status' | 'expirationDate' | 'createdAt' | 'usageCount';
type SortDirection = 'asc' | 'desc';

export const ActivationCodeTable: React.FC<ActivationCodeTableProps> = ({
  codes,
  selectedCodes,
  onSelectionChange,
  onBulkDelete,
  ...
  const getPackageBadge = (packageName: string, packageId?: string) => {
    // Enhanced package badge with soft colors
    // For discount codes, we might have multiple packages in the future
    // For now, just show the single package name
    return (
          ...
          <div className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600 dark:text-gray-300">
                {selectedCodes.length} activation code(s) selected
              </span>
        ...
        <div className="rounded-lg">
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50 dark:bg-gray-900/50 h-12">
                ...
                <TableHead 
                  className="font-semibold text-gray-900 dark:text-gray-100 h-12 cursor-pointer select-none min-w-[280px]"
            ...
            <TableBody>
              {currentCodes.map((code) => (
                <TableRow key={code.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors h-16">
                  ...
                  <TableCell className="h-16">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center space-x-2 flex-1 min-w-0">
                        <span className="font-mono text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded truncate">
                          {code.code}
                        </span>
                        <Button
                          variant="ghost"
                  ...
                  <TableCell className="h-16">
                    {getPackageBadge(code.packageName, code.packageId)}
                  </TableCell>
                  <TableCell className="h-16">
          ...
          {codes.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No activation codes</h3>
